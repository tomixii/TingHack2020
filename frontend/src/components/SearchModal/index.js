...
const SearchModal = ({ open, onClose, firebase }) => {
		...
		<Modal open={open} onClose={onClose} className={classes.paper}>
			<div className={classes.modal}>
				...
				<form className={classes.root} noValidate autoComplete="off" onSubmit={() => handleSubmit()}>
					...
					<div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '20px 40px 20px 40px '}}>
					...
					<FormControl component="fieldset">
						<FormLabel style={{ color: '#EDEAE5' }} className={classes.center} component="legend">Hae</FormLabel>
						<RadioGroup style={{ marginLeft: '40px' }} aria-label="place" name="gender1" value={place} onChange={(event) => setPlace(event.target.value)}>
							<FormControlLabel style={{color: '#EDEAE5'}} value="All" control={<Radio classes={{root: classes.radio, checked: classes.checked}}/>} label="Kaikkialta" />
							<FormControlLabel style={{color: '#EDEAE5'}} value="Own" control={<Radio classes={{root: classes.radio, checked: classes.checked}}/>} label="Omista yhteisöistä" />
						</RadioGroup>
					</FormControl>
					<br></br>
					<div className={classes.center}>
